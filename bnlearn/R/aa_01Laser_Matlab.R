as.bnet <- function(bn_trans.fit){
  
  num_nodes<-length(names(bn_trans.fit))
  #Intra Matrix
  intra <- matrix(data = 0, nrow = num_nodes/2, ncol = num_nodes/2)
  colnames(intra) <- names(bn_trans.fit)[1:(num_nodes/2)]
  rownames(intra) <- names(bn_trans.fit)[1:(num_nodes/2)]
  for (node in rownames(intra)) {
    for (child in bn_trans.fit[[node]]$children)
      intra[node, child] <- 1
  }
  #Inter Matrix
  inter <- matrix(data = 0, nrow = num_nodes/2, ncol = num_nodes/2)
  colnames(inter) <- names(bn_trans.fit)[1:(num_nodes/2)]
  rownames(inter) <- names(bn_trans.fit)[-(1:(num_nodes/2))]
  for (node in rownames(inter)) {
    for (child in bn_trans.fit[[node]]$children)
      inter[node, child] <- 1
  }
  ...
  for(i in 1:(length(names(bn_trans.fit))/2)){
    ns[i] <- dim(bn_trans.fit[[i]]$prob)[1]
  }
  ...
  setVariable(matlab, intra=intra)
  setVariable(matlab, inter=inter)
  ...
  setVariable(matlab, names=names(bn_trans.fit)[1:(num_nodes/2)])
  setVariable(matlab, eclass1=eclass1)
  setVariable(matlab, eclass2=eclass2)
    #Necesary to traspose vectors later on
  evaluate(matlab, "ns=ns';, dnodes=dnodes';, names=names';, eclass1=eclass1';, eclass2=eclass2';, onodes=onodes';")
  ...
  for (i in 1:(num_nodes/2)){ #slice t0
    dimension_node <- dim(bn_trans.fit[[i]]$prob)[1]
    dimension_parents <- dim(bn_trans.fit[[i]]$prob)[-1]
    parents_node <- bn_trans.fit[[i]]$parents
  ...
  for (i in 1:(num_nodes/2)){ #slice t1
    order_bnt <-c()
    parents_node <- bn_trans.fit[[i]]$parents
    parent.in_t <- which(bn_trans.fit[[i]]$parents %in% names(bn_trans.fit)[1:(num_nodes/2)])
    parent.in_tpast <- which(bn_trans.fit[[i]]$parents %in% names(bn_trans.fit)[-(1:(num_nodes/2))])
    order_bnt <-c(parent.in_tpast, parent.in_t) +1
    order_bnt <-c(order_bnt,1) #In BNT the last index if for de node
    CPD_t <-aperm(bn_trans.fit[[i]]$prob, order_bnt)
  ...
  #  evaluate(matlab, "save('/home/aogbechie/Desktop/Datasets/Laser/MATLAB/bnet_data.mat');")
  #  evaluate(matlab, "save('/home/aogbechie/sources/bnlearn/bnet_data.mat')")
#bnet = mk_dbn(intra, inter, ns, 'discrete', dnodes, 'observed', onodes, 'eclass1', eclass1, 'eclass2', eclass2, 'names',names);
#for i=1:num_nodes,bnet.CPD{i} = tabular_CPD(bnet, i);,end
#for i=1:num_nodes,bnet.CPD{i} = tabular_CPD(bnet, i, CPDs{i});,end
#
#  
  #######################
  ...
  T <- 21500 #pass through argument to the function
  setVariable(matlab, T=T)
  system.time(evaluate(matlab, "ev = sample_dbn(bnet, T);"))
  evaluate(matlab, "ll = logLik_transition(bnet,ev)")
  ll<-getVariable(matlab,"ll")
  
  
  #prueba loglikelihood
  #eval(matlab, )
  evaluate(matlab, "onodes=[931,933,934,953,967,981,983,985,987];")
  ...
  evaluate(matlab, "logLik(bnet,cases{1})")
  "engine = smoother_engine(jtree_2TBN_inf_engine(bnet));"
  "[~,ll_seq_prueba]=enter_evidence(engine,cases{1})"
