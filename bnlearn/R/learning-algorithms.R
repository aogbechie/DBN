...
bnlearn = function(x, cluster = NULL, whitelist = NULL, blacklist = NULL,
    ...
    optimized = TRUE, strict = TRUE, undirected = FALSE, simetry = TRUE, cpc_t = NULL) {
  ...
  if (method == "gs") {
    ...
    if (optimized) {
      ...
      mb = maxmin.pc.optimized(x = x, whitelist = whitelist,
             blacklist = full.blacklist, test = test, alpha = alpha, B = B,
             strict = strict, debug = debug, simetry = simetry, cpc_t = cpc_t)
      ...
      mb = maxmin.pc(x = x, whitelist = whitelist, blacklist = full.blacklist,
             ...
             cluster = cluster, simetry = simetry, cpc_t = cpc_t)
...
# static mmpc for DBNs. Skeleton learning withm mmpc.
# if cpc_t is NULL then it learns the nbr like mmpc
# else it learns the neighbourhood of vars in X[t] using an initilized CPC

dyn_nbr = function(x, cluster = NULL, whitelist = NULL, blacklist = NULL,
                   test = NULL, alpha = 0.05, B = NULL, method = "mmpc", debug = FALSE,
                   optimized = TRUE, strict = TRUE, undirected = FALSE, simetry = TRUE, cpc_t = NULL) {
